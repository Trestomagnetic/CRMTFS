...
namespace CrmPlayground
{
    public class SwatTicketSync
    {
        ...
        {
            var teamServer = new TfsKick.Team();
            ...
            foreach (var ticket in ticketsToUpdate)
            {
                ...
                {
                    swatWorkItem = teamServer.GetTfsWorkItemByItemId(tfsId);
                }

                if (swatWorkItem?.Id != null)
                    UpdateWorkItemFromSwatTicket(swatWorkItem, ticket);
                else
                    swatWorkItem = CreateNewWorkItemFromSwatTicket("Bug", ticket);
        ...
        public void SyncSingle(Guid ticketGuid)
        {
            var teamServer = new TfsKick.Team();
        ...
        public static TfsWorkItem CreateNewWorkItemFromSwatTicket(string workItemType, cv_swatticket ticket)
        {
        ...
        private static void UpdateWorkItemFromSwatTicket(TfsWorkItem workItem, cv_swatticket ticket)
        {
