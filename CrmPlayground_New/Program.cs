...
namespace CrmPlayground
{
    ...
    public class RealProgram
    {
        ...
        private static void PrintOptions()
        {
            ...
            Console.WriteLine("   SYNC     SYNC dirty SWAT tickets with the SkyKick 1 team project.");
        ...
        private IOrganizationService CreateTestCrmConnection()
        {
            ...
            if (!string.IsNullOrEmpty(Settings.Default.OverridePassword))
                clientCredentials.UserName.Password = Settings.Default.OverridePassword;
            else
            {
            ...
            var crmUri = new Uri(Settings.Default.CrmOrgServiceUrl);
        ...
        private void GetSwatTicketById(string input)
        {
            var guidId = input.Remove(0, 4).Trim();
            var ticket = CVSwatTicket.GetSwatTicketById(new Guid(guidId));
            QueryInProgress = false;

            var ticketResult = ticket.Attributes["ownerid"];
            ////        continue;
        ...
        private async void SyncSwatTickets(string input)
        {
            var guidId = input.Remove(0, 4).Trim();
            ...
            try
            {
                var swatTickets = new SwatTicketSync();
                await swatTickets.SyncSingle(new Guid(guidId));
                QueryInProgress = false;
                Console.WriteLine();
            }
            catch (Exception ex)
        ...
        private void SyncSwatTickets()
        {
            try
            {
                var swatTickets = new SwatTicketSync();
                swatTickets.SyncDirty();
                QueryInProgress = false;
            }
            catch (Exception ex)
            {
